...
#[allow(unused)]
async fn tcp_listener(mut stream: TcpStream, upstream_passer_send: Sender<UpStreamMessage>, close_passer_send: Sender<CloseSocketMessage>, message_passer_passer_send: Arc<BroadcastSender<client_transit::DownstreamBackpasser>>) {
    ...
    if let Err(error) = stream.read_exact(&mut fixed_header).await {
        println!("Failed to read SOCKS request header: {:?}", error);
        return;
    }
    ...
    loop {
        if buf.len() >= MAX_SOCKS_REQUEST_LENGTH {
            println!("SOCKS request exceeded {} bytes", MAX_SOCKS_REQUEST_LENGTH);
            return;
        }
        ...
        let byte = match stream.read_u8().await {
            Ok(byte) => byte,
            Err(error) => {
                println!("Failed to read SOCKS user ID: {:?}", error);
                return;
            }
        };
